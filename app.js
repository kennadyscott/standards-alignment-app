...
const APP_BUILD = '202608272241';   // replaced with the deploy stamp
...
const STATE_SUBDOMAINS = {
  TX: {
    ...
    byGrade: {
      ...
      '7': {
        /* Its own combination again: the four-subtopic Science list of 3/4/6, but the
           comma-less "Science Technology and Society" of 2/5, grade 6's Fine Arts
           wording, and NO Technology Applications topic at all.

           NOTE: "Physial Health and Hygiene" is misspelled IN THE CMS (missing the c).
           It is reproduced exactly, because the export has to match the string the CMS
           actually stores — if that typo is corrected there, correct it here too. */
        groups: [
          ['Science', ['Matter and Energy', 'Force, Motion, and Energy', 'Earth and Space', 'Organisms and Environments']],
          ['Social Studies', ['History', 'Geography', 'Government', 'Economics', 'Citizenship', 'Culture', 'Science Technology and Society']],
          ['Health', ['Physial Health and Hygiene', 'Mental Health and Wellness', 'Healthy Eating and Physical Activity']],
          ['Personal Financial Literacy', ['Personal Financial Literacy']],
          ['Physical Education', ['Health and Fitness', 'Social and Emotional Health']],
          ['Fine Arts', ['Historical and Cultural Relevance']],
        ],
...
function toCmsRow(dom, rows) {
  ...
  // Only fold when the grade offers a SINGLE science row -- Georgia grade 6 has just
  // "Science", so our Earth/Life/Physical sets belong in it. Where the CMS names science
  // specifically as well, our sets stay in their own row.
  if (isScienceName(dom)) {
...
/* The CMS names some sub-topics differently from the dashboard rows, and the mismatch is
   ...
   there is "Earth Science", so a naive lookup showed 0 and quietly lost them.
...
function cmsSubtopicToRow(cmsName, rowsAtGrade) {
  ...
  // A science name with no row of its own lands in the grade's single science row, when
  // there is exactly one. Where the CMS names science both ways, each keeps its own row.
  if (isScienceName(cmsName)) {
    const sci = rowsAtGrade.filter(isScienceName);
    if (sci.length === 1) return sci[0];   // exactly one, so no guessing
  }
  return null;
}
function cmsLookup(bucket, subdomain, rowsAtGrade) {
  if (!bucket || !bucket.counts) return null;
  const rows = rowsAtGrade || [subdomain];
  let n = 0;
  Object.entries(bucket.counts).forEach(([name, c]) => {
    if (cmsSubtopicToRow(name, rows) === subdomain) n += (+c || 0);
  });
  return n;
}
// CMS sub-topics with no row to land in at this grade — reported, never swallowed.
function cmsUnmatched(bucket, rowsAtGrade) {
  if (!bucket || !bucket.counts || !rowsAtGrade) return [];
  return Object.entries(bucket.counts)
    .filter(([name, c]) => (+c || 0) > 0 && !cmsSubtopicToRow(name, rowsAtGrade));
}
    ...
    wrap.appendChild(el(`
      <div class="dash-card open">
        ...
        ${(() => {
          ...
          return `<div class="cms-orphans">In the CMS with no matching row here:
            ${Object.entries(merged).map(([n, c]) => `<span class="chip">${esc(n)} ${c}</span>`).join(' ')}</div>`;
        })()}
      </div>`));
  });
